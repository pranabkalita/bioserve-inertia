...
GenerateUidList() {
  ...
  then
    ...
    then
      ...
      # so $ids must be used, if populated, before $rest and $input
      echo "$ids"
    elif [ -n "$rest" ]
    then
      echo "$rest"
  ...
  then
    ...
    case "$dbase" in
      ...
      protein )
        ...
        ;;
      snp )
        snpUidList=$( GenerateUidList "$dbase" )
        ...
        ;;
      taxonomy )
        taxUidList=$( GenerateUidList "$dbase" )
    ...
    if [ -n "$fld" ]
    then
      otherUidList=$( GenerateUidList "$dbase" )
